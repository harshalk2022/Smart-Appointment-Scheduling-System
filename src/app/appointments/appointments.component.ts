...
export class AppointmentsComponent implements OnInit {
  ...
  loadProviders() {
    this.http
      .get<any[]>("http://localhost:8080/providers")
      .subscribe((data: any[]) => {
        this.providers = data;
      });
  }
  ...
  loadAppointmentsForProvider() {
    ...
    this.http
      .get<any[]>(`http://localhost:8080/appointments/provider/${providerId}`)
      .subscribe((data: any[]) => {
        this.appointments = data;
      });
  }
  ...
  updateStatus(appointmentId: number, status: string) {
    this.http
      .put(`http://localhost:8080/appointments/${appointmentId}/status`, {
        status,
      })
      .subscribe({
